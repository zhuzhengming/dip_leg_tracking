...
void drawHist(vector<int> nums)
{
    ...
    {
        pts[i].x = i * 2 + 100;
        pts[i].y = 550 - int(nums[i] * (300.0 / (*max_element(nums.begin(), nums.end())))); //归一化到[0, 300]
        //显示横坐标
        if ((i + 1) % 16 == 0)
        {
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		imshow("src", src_frame);//原图

		cvtColor(src_frame,grey_frame,COLOR_BGR2GRAY);//转化为灰度图
		imshow("grey", grey_frame);
		// 此处为实验部分，请自行增加直方图均衡化的代码

		equal_frame = MyequalizeHist(grey_frame);
		imshow("equalize",equal_frame);
		
		
		// openCVHist(src_frame);
		openCVHist(equal_frame);
